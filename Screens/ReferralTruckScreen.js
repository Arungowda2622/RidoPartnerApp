...
const ReferralTruckScreen = ({ route }) => {
  ...
  const fetchStats = async () => {
    try {
      // Fetch user's referral stats for Truck
      const statsResponse = await getVehicleReferralDetails("Truck");
      if (statsResponse && statsResponse.success) {
        setReferralStats(statsResponse.data);
      }
      setLoading(false);
    } catch (error) {
      console.error("Error fetching Truck referral stats:", error);
      setLoading(false);
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch campaign details with milestones for Truck
      const campaignResponse = await getCampaignDetails("Truck");
      ...
      console.error("Error fetching Truck referral data:", error);
  ...
  const handleShare = async () => {
    try {
      ...
      await Share.share({
        message: `🚚 Join RidoDrop as a Truck driver and start earning! 💰\n\nUse my referral code: ${referralCode}\n\nGet amazing rewards! Download the app now! 🚀`,
      });
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <HeaderWithBackButton title="Truck Referral" />
      <ScrollView
...
export default ReferralTruckScreen;
...
//         </TouchableOpacity>
//         <Text style={styles.title}>Truck Referral</Text>
...
//             end={{ x: 1, y: 0 }}
//           >
//             <Icon name="monetization-on" size={28} color="#EC4D4A" style={styles.moneyIcon} />
//             <Text style={styles.earnText}>Earn up to ₹25,000</Text>
//             <Text style={styles.earnSubText}>Per Truck Driver Referral</Text>
//           </LinearGradient>
//         </View>
...
//                 <Text style={styles.stepNumberText}>1</Text>
//               </View>
//               <Text style={styles.stepText}>Share your referral code with truck drivers</Text>
//             </View>
//             <View style={styles.step}>
//               <View style={styles.stepNumber}>
//                 <Text style={styles.stepNumberText}>2</Text>
//               </View>
//               <Text style={styles.stepText}>Friend registers and activates as a truck driver</Text>
//             </View>
//             <View style={styles.step}>
//               <View style={styles.stepNumber}>
//                 <Text style={styles.stepNumberText}>3</Text>
